...
func (c *VolumeSnapshotClassController) syncVolumeSnapshotClass(ctx context.Context, opSpec *operatorapi.OperatorSpec, assetFile string) error {
	...
	if apierrors.IsNotFound(err) {
		...
		if vsc.Name == "csi-azurefile-vsc" {
			fmt.Sprintf("Azure File snapshots are TechPreview in 4.18 - skipping creation of volume snapshot class %s to prevent data loss. See the following KCS for details and snapshot removal guide: <TBD>", vsc.Name)
			return nil
		}
	...
	if existingVSC.Name == "csi-azurefile-vsc" {
		fmt.Sprintf("Azure File snapshots are TechPreview in 4.18 - removing volume snapshot class %s to prevent data loss. See the following KCS for details and snapshot removal guide: <TBD>", vsc.Name)
