...
- Multiple CSI driver operators (AWS EBS/EFS, Azure Disk/File, OpenStack Cinder/Manila, SMB)
...
- Generator processes templates and produces manifests in `pkg/generated-assets/` (embedded in binaries)
...
This runs the generator which processes driver configurations from `pkg/driver/*/` and templates from `assets/` to generate final manifests in `pkg/generated-assets/`.
...
- Controller metrics: 10000-11000 range
- Node metrics: 11000-12000 range
- Liveness probes: 9800-9900 range

Always check the port registry before assigning new ports.
