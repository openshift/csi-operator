...
for i in {0..1}
do
    PACKAGE_MANIFEST=config/${PLATFORMS_NAMES[i]}/manifests/${PLATFORMS_ACRONYMS[i]}-csi-driver-operator.package.yaml
    ...
    echo "Updating ${PLATFORMS_NAMES[i]} package manifest to ${PACKAGE_VERSION}"
    yq -i '.channels[0].currentCSV = strenv(NEW_CURRENT_CSV)' ${PACKAGE_MANIFEST}

    echo "Updating ${PLATFORMS_NAMES[i]} OLM metadata to ${PACKAGE_VERSION}"
    yq -i '
